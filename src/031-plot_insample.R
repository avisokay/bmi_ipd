...
df = read.csv("../results/insample_estimates_10.csv")
...
create_dotplot = function(data, x_var, height_ratio = 0.7) {
  ...
  dodge_width = 0.6  # Reduced from 0.8
  ...
  p = ggplot() +
    ...
    aes(xmin = -Inf, xmax = Inf, 
        ymin = y - 0.5 * height_ratio, ymax = y + 0.5 * height_ratio,  # Apply height ratio
        fill = factor(y %% 2)),
    ...
    # Add color scale matching the density plot colors, with IPD models in black
    ...
    labs(
      title = paste("In-sample estimates for", x_var, "10% labeled."),
      x = "Odds Ratio (with 95% CI)",
      y = NULL
    )
  
  # Improve the legend layout with larger spacing but smaller than before
  p = p + guides(
...
# Generate all plots using the updated create_dotplot function
...
# Save individual plots in landscape orientation if needed
# save_dotplot(df, "race", "../results/race_plot_landscape.pdf", width = 11, height = 8.5)
...
pdf("../results/insample10_plots.pdf", width = 14, height = 10, paper = "special")
for (p in all_plots) {
  print(p)
}
dev.off()
